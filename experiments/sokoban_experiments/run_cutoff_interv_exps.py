...
if __name__ == "__main__":
    ...
    for seed in list(range(args.num_seeds)):
        ...
        for l in range(args.num_layers):
            ...
            probe.load_state_dict(torch.load(f"./convresults/models/tracked_box_next_push_onto_with/{args.model_name}_layer{l}_kernel1_wd0.001_seed{seed}.pt", map_location=torch.device('cpu')))
        ...
        for l in range(args.num_layers):
            probe = ConvProbe(32,5, 1, 0)
            probe.load_state_dict(torch.load(f"./convresults/models/agent_onto_after/{args.model_name}_layer{l}_kernel1_wd0.001_seed{seed}.pt", map_location=torch.device('cpu')))
