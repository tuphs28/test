...
from thinker.core.rnn import ConvAttnLSTM, ResBlock
...
class DRCNet(ActorBaseNet):
    def __init__(self, obs_space, action_space, flags, tree_rep_meaning=None, record_state=False, num_layers=3, num_ticks=3, input_dim=7):
        ...
        self.core = ConvAttnLSTM(            
            input_dim=32,
            hidden_dim=32,
            num_layers=self.num_layers, # I CHANGED THIS
            attn=False,
            h=h,
            w=w,            
            kernel_size=3,
    ...
    def forward(self, env_out, core_state=(), clamp_action=None, compute_loss=False, greedy=False, ood=False):
        done = env_out.done
        assert (
            len(done.shape) == 2
        ), f"done shape should be (T, B) instead of {done.shape}"
...
class ResNet(ActorBaseNet):
    ...
    def __init__(self, obs_space, action_space, flags, tree_rep_meaning=None, record_state=False, in_dim=7, hidden_dim=32, num_layers=8):
        ...
        self.encoder = nn.Conv2d(in_channels=in_dim, out_channels=hidden_dim, kernel_size=3, padding=3//2)
        in_dim = channels[0]
        for (channel, kernel_size, stride) in zip(channels, kernel_sizes, strides):
            layers += [
                ResBlock(in_dims=in_dim, out_dims=channel, kernel_size=kernel_size, stride=stride, record_state=record_state)
            ]
            in_dim = channel

        self.core = nn.Sequential(*layers)
        self.record_state = record_state
        h, w = obs_space["real_states"].shape[-2], obs_space["real_states"].shape[-1]
        def compute_output_shape(h, w, layers):
            for kernel_size, stride, padding in layers:
                h = (h + 2 * padding - kernel_size) // stride + 1
                w = (w + 2 * padding - kernel_size) // stride + 1
            return h, w
        final_h, final_w = compute_output_shape(h, w, [(k, s, k//2) for k, s in zip(kernel_sizes, strides)])

        self.final_layer = nn.Linear(final_h*final_w*channel, 256)
        self.policy = nn.Linear(256, self.num_actions * self.dim_actions)
        self.baseline = nn.Linear(256, 1)
...
def ActorNet(*args, **kwargs):
    ...
    if getattr(kwargs["flags"], "drc", False):        
        ...
        Net = MCTS
    elif getattr(kwargs["flags"], "resnet", False): 
        Net = ResNet
